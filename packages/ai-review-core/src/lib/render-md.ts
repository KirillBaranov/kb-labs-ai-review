...
function groupFindings(findings: ReviewFinding[], order: Severity[]): Grouped {
  ...
  for (const key of Object.keys(g) as Severity[]) {
    if (g[key]) {
      g[key].sort(
        (a, b) =>
          (a.area || '').localeCompare(b.area || '') || (a.file || '').localeCompare(b.file || '')
      );
    }
  }
...
export function renderMarkdown(findings: ReviewFinding[], opts: RenderOptions = {}): string {
  ...
  for (const s of order) {
    ...
    for (const [k, list] of byArea) {
      ...
      for (const f of list) {
        ...
        out.push(`  - ${what}`);
        if (f.why) {
          out.push(`  - _Why:_ ${f.why}`);
        }
        if (f.suggestion) {
          out.push(`  - _Fix:_ ${f.suggestion}`);
        }
      }
    }
  }
