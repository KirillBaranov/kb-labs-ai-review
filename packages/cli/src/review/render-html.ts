...
export async function renderHtmlCLI(opts: { inFile: string; outFile: string; repoRoot?: string }) {
  const repoRoot = opts.repoRoot || await findRepoRootAsync()
  const inAbs  = path.isAbsolute(opts.inFile)  ? opts.inFile  : path.join(repoRoot, opts.inFile)
  const outAbs = path.isAbsolute(opts.outFile) ? opts.outFile : path.join(repoRoot, opts.outFile)
